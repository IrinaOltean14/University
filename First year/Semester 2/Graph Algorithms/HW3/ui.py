...
class UI:
    ...
    def print_menu():
        print("\nCHOOSE YOUR OPTION\n"
              ...
              "19. Find shortest path between two vertices (using Bellman Ford algorithm)\n"
              "20. Exit")
    ...
    def find_shortest_path(self):
        ...
        if not self._graph.check_if_vertex_in_graph(int(start)) or not self._graph.check_if_vertex_in_graph(int(end)):
            print("One of the vertices you enter is not in the graph\n")
        else:
            try:
                distance, path = self._graph.efficient_bellman_ford(int(start), int(end))
